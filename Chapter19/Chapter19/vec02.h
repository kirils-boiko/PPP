...
/*	When compared to v00.h, this version uses the std::unique_ptr for managing
*	the memory in the reserve();
*/
...
template<class T, class A> void vector<T, A>::reserve(int newalloc)
{
	...
	std::unique_ptr<T> p{ new T[newalloc] };				// allocate new space
	for (int i = 0; i < sz; ++i) p.get()[i] = elem[i];	// copy old elements
	delete[] elem;									// destroy old elements
	elem = p.release();
	space = newalloc;
}
